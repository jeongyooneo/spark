...
private[spark] class BlazeCostLeafCntAnalyzer(val rddJobDag: RDDJobDag,
  ...
  override def compDisaggCost(blockId: BlockId): CompDisaggCost = {
    val refStages = rddJobDag.getLeaf(blockId)
    ...
    if (refStages.isEmpty) {
      ...
      new CompDisaggCost(blockId, 0, recompTime * refStages.size)
    }
  }
