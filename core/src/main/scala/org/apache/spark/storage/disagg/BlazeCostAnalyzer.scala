...
private[spark] class BlazeCostAnalyzer(val rddJobDag: RDDJobDag,
  ...
  override def compDisaggCost(blockId: BlockId): CompDisaggCost = {
    ...
    val disaggOverhead = disaggCostCalc(blockId, metricTracker.getBlockSize(blockId), refCnt)
    new CompDisaggCost(blockId, disaggOverhead, refCnt * recompTime)
  }
}
