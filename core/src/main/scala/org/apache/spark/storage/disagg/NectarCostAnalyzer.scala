...
private[spark] class NectarCostAnalyzer(val rddJobDag: RDDJobDag,
  ...
  private def calculateCostBenefitRatio(blockId: BlockId, ct: Long): Long = {
    ...
    val cost = (metricTracker.localBlockSizeHistoryMap
      .get(blockId) / 10000) * (ct - nectarInfo.refTime)
    val benefit = Math.max(1, nectarInfo.refCnt) * recompTime
    cost / benefit
  }
  ...
  override def compDisaggCost(blockId: BlockId): CompDisaggCost = {
    ...
    val c = new CompDisaggCost(blockId, 0, calculateCostBenefitRatio(blockId,
      System.currentTimeMillis()))
    c
  }
}

class NectarInfo(var refTime: Long,
                 var refCnt: Int) {
