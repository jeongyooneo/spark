...
private[spark] class MemoryStore(
  ...
  val disaggManager = disaggStore.disaggManager

  @transient lazy val mylogger = org.apache.log4j.LogManager.getLogger("myLogger")

  private var decisionByMaster = true
  ...
  private def sizeEstimation[T](values: Iterator[T],
                                newValues: ListBuffer[T],
                                classTag: ClassTag[T],
                               valuesHolder: ValuesHolder[T]): Long = {
    // Size estimation !!
    while (values.hasNext) {
      val v = values.next()
      newValues.append(v)
      valuesHolder.storeValue(v)
    }
  ...
  Either[(Long, Long), (MemoryEntry[T], Long, Long)] = {
    ...
    var memoryThreshold = initialMemoryThreshold + size
    ...
    keepUnrolling =
      reserveUnrollMemoryForThisTask(blockId, memoryThreshold, memoryMode)
    val evictEnd = System.currentTimeMillis()
    val evictInit = evictEnd - evictStart

    if (!keepUnrolling) {
      logWarning(s"Failed to reserve initial memory threshold of " +
        s"${Utils.bytesToString(memoryThreshold)} for computing block $blockId in memory.")
    } else {
      unrollMemoryUsedByThisBlock += (memoryThreshold)
    }
